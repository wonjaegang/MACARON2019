...
def get_already_fit_image_plot(out_img_half_sum, left_fit, right_fit, center_fit, road_width, PIXEL_CONVERSION_RATE, roi_limit_M, roi_limit_L, roi_limit_R, zero_1, zero_3, PT_view_height, normal_mid_left_contactx, normal_mid_right_contactx, normal_mid_left_contacty, normal_mid_right_contacty, normal_top_left_contactx, normal_top_right_contactx, normal_top_left_contacty, normal_top_right_contacty, margin=50, is_plot=True):
    ...
    fillPoly(window_img_full, np.int_([left_line_pts]), (0, 255, 0))
    fillPoly(window_img_full, np.int_([right_line_pts]), (0, 255, 0))
    fillPoly(window_img_left, np.int_([left_line_pts]), (1))
    fillPoly(window_img_right, np.int_([right_line_pts]), (1))
    fillPoly(window_img_left_submargin, np.int_([left_line_pts_submargin]), (1))
    fillPoly(window_img_right_submargin, np.int_([right_line_pts_submargin]), (1))  # 필요하면(간소화 등) 특정 경우에만 코드가 돌도록 수정
    ...
    circle(result, ((int)(normal_mid_left_contactx), (int)(normal_mid_left_contacty)), 3, (0, 0, 255), -1)
    circle(result, ((int)(normal_mid_right_contactx), (int)(normal_mid_right_contacty)), 3, (0, 0, 255), -1)
    circle(result, ((int)(normal_top_left_contactx), (int)(normal_top_left_contacty)), 3, (0, 0, 255), -1)
    circle(result, ((int)(normal_top_right_contactx), (int)(normal_top_right_contacty)), 3, (0, 0, 255), -1)
...
def plot_lanes_unwrap(viewsize, left_poly, right_poly, left_turn, right_turn, left_yellow_lane, right_yellow_lane, undistorted_in_rgb, M_INV, stopx_base, roi_limit_M, roi_limit_L, roi_limit_R, originalsize, is_plot=False, add_debug_image=True):
    ...
    for pts, index in enumerate(center_fitxxx):
        circle(color_warp, ((int)(center_fitxxx[pts]), (int)(plot_yyy[pts])), 2, (0, 155, 255), -1)
...
class image_converter:
    ...
    def lanetracker(self):
        ...
        while True:  # @@@@@@@@@@@@@@@@@@@@영상출력루프 시작@@@@@@@@@@@@@@@@@@@@@@
            ...
            a = 'time_diff = ' + '{:0.4f}'.format(current_time - prev_time) + '   Hz = ' + '{:}'.format(1/ (current_time - prev_time))
            # a = "tresh_left : " + "{:0.2f}".format(white_tresh_lower_left) + " "
            print(a)
            #print(a, )
...
# ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ1차 로스 추가용 코드ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
            ...
            lower_yellow = (1.5, yellow_s_tresh_lower, 80)  # HSV 컬러 영억 검출 임계값 영역 (색상, 채도, 조도)
            ...
            # final_line_bynary_full = cv2.bitwise_or(color_bynary, binary_detetct_morph_brg_full)
...
# ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ2차 로스 추가용 코드ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
            ...
            if add_debug_image3:
                ...
                putText(composed_image, following_lane_text, (580, 210), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
